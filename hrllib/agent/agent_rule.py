...
class AgentRule(Agent):
    ...
    def __init__(self,action_set, slot_set, disease_symptom, parameter):
        super(AgentRule,self).__init__(symptoms=action_set,conditions=slot_set,parameter=parameter)
    ...
    def next(self, state, turn, greedy_strategy, **kwargs):
        ...
        if len(candidate_symptoms) == 0:
            ...
            self.agent_action["request_slots"].clear()
            self.agent_action["request_slots"][symptom] = configuration.VALUE_UNKNOWN
    ...
    def _get_candidate_disease_symptoms(self, state):
        ...
        for disease in self.disease_symptom.keys():
            ...
            disease_match_number[disease]["not"] = 0


        for slot in inform_slots.keys():
            for disease in disease_match_number.keys():
                if inform_slots[slot] in self.disease_symptom[disease]["symptom"] and inform_slots[slot] == False:
                    disease_match_number[disease]["not"] += 1
                else:
        ...
        for disease in disease_match_number.keys():
            ...
            not_sure_score = float(disease_match_number[disease]["not"]) / len(self.disease_symptom[disease]["symptom"])
            disease_score[disease] = yes_score - not_sure_score 
