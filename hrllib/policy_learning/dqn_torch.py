...
class DQN(object):
    def __init__(self, input_size, hidden_size, output_size, parameter, named_tuple=('state', 'agent_action', 'reward', 'next_state', 'episode_over')):
        ...
        self.current_net = MLP(input_size, hidden_size, output_size, parameter).to(self.device)
        self.target_net = MLP(input_size, hidden_size, output_size, parameter).to(self.device)
        if torch.cuda.is_available():
    ...
    def save_model(self, model_performance,episodes_index, checkpoint_path):
        ...
        average_match_rate = model_performance["average_match"]
        top3 = model_performance["top3"]
        ...
        model_file_name = os.path.join(checkpoint_path, "model_d" + str(disease_number) +  str(agent_id) + "_s" + str(success_rate) + "_r" + str(average_reward) + "_t" + str(average_turn)\
                          + "_mr" + str(average_match_rate) + "_mr2-" + str(top3) +str(top5) + "_e-" + str(episodes_index) + ".pkl")
    ...
    def restore_model(self, saved_model):
        ...
        """
        model_file = [f for f in os.listdir(saved_model) if f.endswith('.pkl')]
        saved_model = saved_model+model_file[0]
